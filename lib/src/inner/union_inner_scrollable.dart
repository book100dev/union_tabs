...
/// UnionInnerScrollable content is displayed.
typedef ViewportBuilder = Widget Function(
    BuildContext context, ViewportOffset position);

/// A widget that scrolls.
///
/// [UnionInnerScrollable] implements the interaction model for a UnionInnerScrollable widget,
...
/// a [GridView].
///
/// To further customize scrolling behavior with a [UnionInnerScrollable]:
///
...
///    that keeps the same page visible when the [UnionInnerScrollable] resizes.
///
/// See also:
///
...
///  * [SingleChildScrollView], which is a UnionInnerScrollable widget that has a single
///    child.
...
class UnionInnerScrollable extends StatefulWidget {
  ...
  /// See also:
  ///
  ///  * [AlwaysUnionInnerScrollableScrollPhysics], which can be used to indicate that the
  ///    UnionInnerScrollable should react to scroll requests (and possible overscroll)
  ///    even if the UnionInnerScrollable's contents fit without scrolling being necessary.
  final ScrollPhysics? physics;

  /// Builds the viewport through which the UnionInnerScrollable content is displayed.
  ///
  ...
  /// scroll when the UnionInnerScrollable is asked to scroll via the keyboard using a
  /// [ScrollActionf].
  ///
  ...
  ///
  /// Text fields with an overflow are usually UnionInnerScrollable to make sure that the
  ...
  /// {@template flutter.widgets.UnionInnerScrollable.dragStartBehavior}
  ...
  /// {@template flutter.widgets.UnionInnerScrollable.restorationId}
  /// Restoration ID to save and restore the scroll offset of the UnionInnerScrollable.
  ///
  /// If a restoration id is provided, the UnionInnerScrollable will persist its current
  ...
  ///
  /// ```dart
  /// UnionInnerScrollableState UnionInnerScrollable = UnionInnerScrollable.of(context);
  /// ```
  ///
  /// Calling this method will create a dependency on the closest [UnionInnerScrollable]
  ...
  ///
  /// The heuristic used is determined by the [physics] of this [UnionInnerScrollable]
  ...
  ///
  /// If there is no [UnionInnerScrollable] in the widget tree above the [context], this
  ...
  /// Scrolls the UnionInnerScrollables that enclose the given context so as to make the
  ...
  static Future<void> ensureVisible(
    ...
    // If there are multiple UnionInnerScrollable widgets nested, we should let
...
// Enable UnionInnerScrollable.of() to work as if UnionInnerScrollableState was an inherited widget.
// UnionInnerScrollableState.build() always rebuilds its _UnionScrollableScope.
...
class UnionInnerScrollableState extends State<UnionInnerScrollable>
  ...
  late TabBarOverScroll _tabBarOverScroll;
  late UnionHorizontalDragGestureRecognizer horizontalDragGestureRecognizer;
  ...
  Widget build(BuildContext context) {
    ...
    // so that we can get this UnionInnerScrollableState by doing the following:
    //
    // ScrollNotification notification;
    // UnionInnerScrollable.of(notification.context)
    //
...
/// Describes the aspects of a UnionInnerScrollable widget to inform inherited widgets
/// like [ScrollBehavior] for decorating.
///
/// Decorations like [GlowingOverscrollIndicator]s and [Scrollbar]s require
/// information about the UnionInnerScrollable in order to be initialized.

//''''''''

/// With [_ScrollSemantics] certain child [SemanticsNode]s can be
/// excluded from the UnionInnerScrollable area for semantics purposes.
///
...
/// two semantics nodes will be used to represent the [UnionInnerScrollable]: The outer
/// node will contain all children, that are excluded from scrolling. The inner
/// node, which is annotated with the scrolling actions, will house the
/// UnionInnerScrollable children.
...
///
/// This function is used as the type for [UnionInnerScrollable.incrementCalculator],
...
/// [ScrollAction] on a [UnionInnerScrollable].
///
/// This is used to configure a [ScrollIncrementDetails] object to pass to a
/// [ScrollIncrementCalculator] function on a [UnionInnerScrollable].
///
...
/// UnionInnerScrollable areas will have the concept of a "line" or "page", but they can
...
enum ScrollIncrementType {
  ...
  /// particular UnionInnerScrollable.
  line,
...
class ScrollIncrementDetails {
  ...
  /// The current metrics of the UnionInnerScrollable that is being scrolled.
  final ScrollMetrics metrics;
}

/// An [Intent] that represents scrolling the nearest UnionInnerScrollable by an amount
/// appropriate for the [type] specified.
///
/// The actual amount of the scroll is determined by the
/// [UnionInnerScrollable.incrementCalculator], or by its defaults if that is not
/// specified.
class ScrollIntent extends Intent {
  ...
  /// The direction in which to scroll the UnionInnerScrollable containing the focused
  /// widget.
...
/// An [Action] that scrolls the [UnionInnerScrollable] that encloses the current
/// [primaryFocus] by the amount configured in the [ScrollIntent] given to it.
///
/// If a UnionInnerScrollable cannot be found above the current [primaryFocus], the
/// [PrimaryScrollController] will be considered for default handling of
/// [ScrollAction]s.
///
/// If [UnionInnerScrollable.incrementCalculator] is null for the UnionInnerScrollable, the default
...
class ScrollAction extends Action<ScrollIntent> {
  ...
  bool isEnabled(ScrollIntent intent) {
    ...
    if (contextIsValid) {
      // Check for primary UnionInnerScrollable within the current context
      if (UnionInnerScrollable.of(focus.context!) != null) return true;
      // Check for fallback UnionInnerScrollable with context from PrimaryScrollController
      if (PrimaryScrollController.of(focus.context!) != null) {
        ...
        return primaryScrollController != null &&
            ...
            primaryScrollController.position.context.notificationContext !=
                null &&
            UnionInnerScrollable.of(primaryScrollController
  ...
  void invoke(ScrollIntent intent) {
    ...
    assert(state != null,
        '$ScrollAction was invoked on a context that has no UnionInnerScrollable parent');
    assert(state!.position.hasPixels,
        'UnionInnerScrollable must be laid out before it can be scrolled via a ScrollAction');
    assert(state!.position.viewportDimension != null);
    assert(state!.position.maxScrollExtent != null);
    assert(state!.position.minScrollExtent != null);
